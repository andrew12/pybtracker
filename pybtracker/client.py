...
class UdpTrackerClientProto(asyncio.Protocol):
    ...
    async def connect(self):
        ...
        if self.received_msg:
            ...
            if action == 3:
                self.logger.warn(
                    "An error was received in reply to connect: {}".format(
                        data.decode()
                    )
                )
    ...
    async def announce(
        self, infohash, num_want, downloaded, left, uploaded, event=0, ip=0
    ):
        if (
            not self.client.interval
            or not self.client.connid
            or datetime.now()
            > self.client.connid_timestamp
        ...
        if self.received_msg:
            ...
            if action == 3:
                self.logger.warning(
                    "An error was received in reply to announce: {}".format(
                        data.decode()
                    )
                )
                raise ServerError(
                ...
                if len(data) < 12:
                    ...
                    return None
                self.client.interval, leechers, seeders = struct.unpack(
                    "!III", data[:12]
                )
            ...
            data = data[12:]
            if len(data) % 6 != 0:
                self.logger.warning("Invalid announce reply received. Invalid length.")
                return None

            peers = [data[i : i + 6] for i in range(0, len(data), 6)]
            peers = [
                (str(ip_address(p[:4])), int.from_bytes(p[4:], byteorder="big"))
                for p in peers
            ]
...
class ClientShell(cmd.Cmd):
    ...
    def do_announce(self, arg):
        ...
        parser.add_argument(
            "infohash",
            type=hex_encoded_infohash,
            help="The infohash of the torrent to announce in hex-encoded " "format.",
        )
        parser.add_argument(
            "downloaded", type=int, help="Downloaded bytes to announce."
        )
        parser.add_argument("left", type=int, help="Left bytes to announce.")
        parser.add_argument("uploaded", type=int, help="Uploaded bytes to announce.")
        parser.add_argument(
            "--num-want",
            "-n",
            type=int,
            default=160,
            help="Maximum number of peers to peers to request. " "Defaults to 160.",
        )
        parser.add_argument(
            "--event",
            "-e",
        ...
        args.event = ["none", "completed", "started", "stopped"].index(args.event)
...
def main():
    ...
    parser.add_argument("tracker_uri", metavar="URI", help="The tracker URI.")
    parser.add_argument(
        "--log-to-stdout",
        "-O",
        action="store_true",
    ...
    parser.add_argument(
        "--version", "-V", action="version", version="pybtracker v" + __version__
    )
