...
class TrackerServerProtocolTest(asynctest.TestCase):
    async def setUp(self):
        ...
        self.proto = pybtracker.server.UdpTrackerServerProto(
            self.server, allowed_torrents={}
        )
    ...
    async def test_process_connect_invalid(self):
        result = self.proto.process_connect(
            ("127.0.0.1", 8007), 0x4444444444, 1000, b""
        )
    ...
    async def test_process_connect_valid(self, mock_datetime):
        ...
        result = self.proto.process_connect(
            ("127.0.0.1", 8007), 0x41727101980, 1000, b""
        )
    ...
    async def test_process_announce_old_connid(self):
        self.server.connids[0x99887766] = datetime.now() - timedelta(
            seconds=self.server.connid_valid_period + 1
        )
        result = self.proto.process_announce(
            ("127.0.0.1", 8007),
            0x99887766,
    ...
    async def _test_process_announce_too_frequent(self, event, mock_datetime):
        ...
        self.server.connids[0x99887766] = now - timedelta(
            seconds=self.server.connid_valid_period - 1
        )
        self.server.activity[0x99887766] = now - timedelta(
            seconds=self.server.interval - 1
        )
    ...
    async def _test_process_announce_valid(self, num_want, mock_datetime):
        ...
        peers = result[20:]
        peers = [
            (peers[i : i + 4], peers[i + 4 : i + 6]) for i in range(0, len(peers), 6)
        ]
        peers = [
            (str(ip_address(ip)), int.from_bytes(port, byteorder="big"))
            for ip, port in peers
        ]
        self.assertLessEqual(len(peers), num_want)
        for i in peers:
            self.assertIn(i, [("1.1.1.1", 1111), ("2.2.2.2", 2222), ("3.3.3.3", 3333)])
        self.assertEqual(self.server.activity[0x99887766], now)

        ip = int.from_bytes(ip_address("127.0.0.2").packed, byteorder="big")
        self.server.announce.assert_called_with(
            b"01234567890123456789",
    ...
    async def test_datagram_received_connect(self):
        ...
        self.proto.process_connect.assert_called_with(
            ("127.0.0.3", 8007), 0x41727101980, 1000, b""
        )
        self.proto.transport.sendto.assert_called_with(
            mock_response, ("127.0.0.3", 8007)
        )
    ...
    async def test_datagram_received_announce(self):
        ...
        self.proto.process_announce.assert_called_with(
            ("127.0.0.3", 8007), 0x123456, 1000, b"X" * 160
        )
        self.proto.transport.sendto.assert_called_with(
            mock_response, ("127.0.0.3", 8007)
        )
...
class TrackerServerTest(asynctest.TestCase):
    async def setUp(self):
        self.server = pybtracker.TrackerServer(local_addr=("127.0.0.1", 0))
    ...
    async def test_init_default(self):
        server = pybtracker.TrackerServer(local_addr=("127.0.0.1", 0))
    ...
    async def _test_announce(self, event, new_torrent, new_peer):
        ...
        if not new_torrent:
            if new_peer:
                self.server.torrents = {
                    ih: {b"11111111111111111111": ("1.1.1.1", 1111, 900, 99, 9, False)}
                }
            else:
                self.server.torrents = {
                    ih: {peerid: ("1.1.1.1", 1111, 900, 99, 9, False)}
                }
        ...
        if event not in [0, 1, 2, 3]:
            if new_torrent:
                ...
                _ip, _port, _dl, _left, _ul, _completed = self.server.torrents[ih][
                    peerid
                ]
        ...
        if event == 3:  # stopped
            if new_torrent:
                ...
                self.assertEqual(
                    list(self.server.torrents[ih].keys()), [b"11111111111111111111"]
                )
            else:
                self.assertEqual(self.server.torrents, {})
            return
...
class TrackerClientTest(asynctest.TestCase):
    ...
    async def test_init_invalid_uri(self):
        with self.assertRaises(ValueError):
            client = pybtracker.TrackerClient("http://sometracker.com:8000/announce")
    ...
    async def test_init_valid_with_max_transmissions(self):
        client = pybtracker.TrackerClient(
            "udp://foo.net/announce", max_retransmissions=10
        )
...
class TrackerClientProtoTest(asynctest.TestCase):
    ...
    async def test_datagram_received_valid(self):
        ...
        self.proto.datagram_received(
            struct.pack("!II", 5, 1000) + b"abcdefgh", ("127.0.0.1", 10000)
        )
    ...
    async def _test_send_msg(self, expected_failures, mock_asyncio):
        ...
        self.assertEqual(
            failures, min(expected_failures, self.client.max_retransmissions)
        )
    ...
    async def _test_announce(self, expect_connect, successful_connect=True):
        ...
        async def send_msg_side_effect(msg, tid):
            (
                ...
                _nw,
                _port,
            ) = struct.unpack("!QII20s20sQQQIIIIH", msg)
    ...
    async def test_announce_connid_expired(self):
        ...
        await self._test_announce(expect_connect=True, successful_connect=True)
    ...
    async def test_announce_cant_connect(self):
        self.client.interval = 120
        self.client.connid = None
        await self._test_announce(expect_connect=True, successful_connect=False)
